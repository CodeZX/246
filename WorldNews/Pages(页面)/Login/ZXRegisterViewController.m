...
- (void)registerAction {
    ...
    [NetManager POSTRegisterPhone:self.phoneTextField.text pwd:self.passwordTextField.text completionHandler:^(XTJRegisterItem *essences, NSError *error) {
...
- (void)didRegisterAction {
    ...
    NSString * sms_content = [NSString stringWithFormat:@"%@是本次操作的手机验证码，如非本人操作，请忽略本短信。",randStr];
...
- (UITextField *)registerTextField {
    if (!_registerTextField) {
        ...
        [button setTitleColor:RGB(0,165,234) forState:UIControlStateNormal];
        [button setTitle:@"获取验证码" forState:UIControlStateNormal];
        button.layer.cornerRadius = 3.0f;
        button.layer.masksToBounds = YES;
        button.layer.borderColor = RGB(0,165,234).CGColor;
