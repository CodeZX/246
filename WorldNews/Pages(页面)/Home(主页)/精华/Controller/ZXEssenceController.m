...
#import "ZXEssenceImageCell.h"
#import "ZXEssenceWebController.h"
@interface ZXEssenceController ()<iCarouselDelegate, iCarouselDataSource>
/** 精华数组 */
@property(nonatomic, strong) NSMutableArray<YGEssenceResponseFeedsItem *> *essenceArr;
/** 模型 */
@property(nonatomic, strong) ZXEssenceItem *essenceItem;
/** 是否还有更多 */
...
@implementation ZXEssenceController
/**
 *  cell重用标示
 */
...
- (void)configNetManager
{
    ...
    [self.tableView addHeaderRefresh:^{
        [NetManager GETEssenceItem:@"0" completionHandler:^(ZXEssenceItem *essences, NSError *error) {
    ...
    [self.tableView addFooterRefresh:^{
        [NetManager GETEssenceItem:weakSelf.lastKey completionHandler:^(ZXEssenceItem *essences, NSError *error) {
    ...
    [self.tableView registerClass:[ZXEssenceImageCell class] forCellReuseIdentifier:@"ZXEssenceImageCell"];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (feedsItem.type == 0) {
        ZXEssenceImageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ZXEssenceImageCell" forIndexPath:indexPath];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (feedsItem.type == 0) {
        return [tableView fd_heightForCellWithIdentifier:@"ZXEssenceImageCell" configuration:^(ZXEssenceImageCell *cell) {
            [cell.iconIV setImageWithURL:feedsItem.image.yg_URL options:YYWebImageOptionIgnoreAnimatedImage];
        }];
    }
...
{
        ZXEssenceWebController *webVC = [[ZXEssenceWebController alloc] initWithAppView:self.bannersArr[index].post.appview];
        [self.navigationController pushViewController:webVC animated:YES];
}
